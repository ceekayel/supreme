...
if($popular_post_query):
	while ($popular_post_query->have_posts()) : $popular_post_query->the_post();
		...
		if($popular_per == 'views' || $popular_per == 'dailyviews'){
			$posthtml .= '<div class="post_data"><h3><a href="'.$guid.'" title="'.$post_title.'">'.$post_title.'</a></h3><p><span class="date">'.$fv."</span><span class='views'> ".$views.'</span></p></div></li>';
		}else{
			$posthtml .= '<div class="post_data"><h3><a href="'.$guid.'" title="'.$post_title.'">'.$post_title.'</a></h3><p><span class="date">'.$fv."</span><span class='views'> ".$comments.'</span></p></div></li>';
		}				
